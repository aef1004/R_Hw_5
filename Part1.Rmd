...
TX_tracts <- tracts("Texas", county = c("Harris County", "Brazoria", "Fort Bend", 
                                        "Montgomery", "Liberty", "Waller"), cb = TRUE) 
...
ggplot(homicides_sf) +
  ...
  geom_sf(data = homicides_sf, aes(color = victim_race_factor), size = .3) +
...
baltimore <- unique(data.frame(event_date = baltimore$dates, event_count = baltimore$n, season = baltimore$season))
...
baltimore %>% 
  ...
  geom_smooth( aes(x = event_date, y = event_count),method="loess", se=FALSE, span=0.1, size = 1.2)+
  labs(x = "Date", y = "Monthly homicides", fill = " ") +
   theme_dark()+
   theme(legend.position = "bottom")+
  ggtitle("Homicides in Baltimore, MD")+
  geom_vline(data = freddie, xintercept = (freddie$dates), color = "red", linetype = "dashed", size = 1.5) +
  geom_text(data = freddie, aes(x=dates, y= n, label="Arrest of 
                                Freddie Gray"), size=7, hjust= 1,
