...
head(Houston_homicides)
# top 3 races are Black, Hispanic, White
```
...
Houston_roads <- primary_secondary_roads(state = "TX")
...
# facet wrap based on if the case is solved or unsolved
ggplot(Houston_homicides) +
  facet_wrap(~unsolved) +
  geom_sf(data = Houston_sf) +
  geom_sf(data = Houston_roads)
